...
def parser(url : str):
    ...
    res = requests.get(f'{url}page/{counter}/')
...
def create_csv():
    with open(f'shoes.csv', mode='w', newline='') as file:
...
def write_csv(products : list[Product]):
    with open(f'shoes.csv', mode='a', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
...
if __name__ == '__main__':
    parser(url = 'https://kicksmania.ru/shop/categories-kicks/')